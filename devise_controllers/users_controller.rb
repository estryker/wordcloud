class UsersController < ApplicationController
 ...
 def show
   ...
   # TODO: try to get @user.items.paginate to work ...
   # @items = Item.where(:user_email => @user.email).paginate(:page => params[:page])
   # doesn't work:
   # @items = Item.where(:user_id => @user.id).order("created_at DESC").paginate(:page => params[:page])
   @available_items = Item.where(:user_id => @user.id, :is_available => true).order("created_at DESC").paginate(:page => params[:page])
   @unavailable_items = Item.where(:user_id => @user.id, :is_available => false).order("created_at DESC").paginate(:page => params[:page])

   # stinkin paginate prevents this awesomeness!!
   # @available_items, @unavailable_items = items.partition {|i| i.is_available }
